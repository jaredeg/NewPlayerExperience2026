...
----------------------------------------------------------------------------
-- Freeze a spawned bot hero offmap so its AI cannot execute until revealed.
-- Call from OnSpawnerFinished for any second-wave spawner that must wait.
----------------------------------------------------------------------------

function CDotaNPXScenario:FreezeSpawnedUnit( hSpawner )
	local hUnit = hSpawner:GetSpawnedUnits()[ 1 ]
	if hUnit then
		local ok, err = pcall( function()
			hUnit.bFrozen = true
			hUnit:AddNewModifier( nil, nil, "modifier_invulnerable", {} )
			hUnit.vSpawnOrigin = hUnit:GetAbsOrigin()
			hUnit:SetAbsOrigin( Vector( 10000, 10000, 0 ) )
		end )
		if not ok then
			printf( "WARNING: FreezeSpawnedUnit failed: %s", tostring( err ) )
		end
	end
end

----------------------------------------------------------------------------
-- Reveal a previously-frozen bot hero at its original spawn position.
-- Sets bFrozen = false LAST so AI Think does not fire before positioning.
----------------------------------------------------------------------------

function CDotaNPXScenario:RevealSpawnedUnit( hSpawner )
	local hUnit = hSpawner:GetSpawnedUnits()[ 1 ]
	if hUnit then
		printf( "RevealSpawnedUnit: %s at %s", hUnit:GetUnitName(), tostring( hUnit.vSpawnOrigin ) )
		hUnit:RemoveModifierByName( "modifier_invulnerable" )
		FindClearSpaceForUnit( hUnit, hUnit.vSpawnOrigin, true )
		hUnit:Stop()
		hUnit.bFrozen = false
	end
end
...
function CDotaNPXScenario:Restart()
	...
	-- NOTE: Do NOT call spawner:RemoveSpawnedUnits() or clear rgSpawners.
	-- Bot heroes are reused across restarts — SetupScenario will reset
	-- them in place instead of destroying and recreating them.
	...
	-- The player hero persists across restarts.  Reset it.
	local hPlayerHero = PlayerResource:GetSelectedHeroEntity( 0 )
	printf( "Restart: PlayerHero = %s, alive = %s", tostring( hPlayerHero ), tostring( hPlayerHero and not hPlayerHero:IsNull() and hPlayerHero:IsAlive() ) )
	if hPlayerHero and not hPlayerHero:IsNull() then
		self:_ResetPlayerHero( hPlayerHero )
	end

	-- Safety: if retry was clicked at the exact moment of death, the hero
	-- might die AFTER our reset (death animation still in flight).
	-- Schedule a safety check one tick later.
	self:ScheduleFunctionAtGameTime( GameRules:GetGameTime() + 0.2, function()
		local hHero = PlayerResource:GetSelectedHeroEntity( 0 )
		if hHero and not hHero:IsNull() and not hHero:IsAlive() then
			printf( "Restart safety: hero still dead after restart, re-respawning" )
			self:_ResetPlayerHero( hHero )
		end
	end )
end
...
function CDotaNPXScenario:_ResetPlayerHero( hPlayerHero )
	if not hPlayerHero:IsAlive() then
		hPlayerHero:RespawnHero( false, false )
	end

	hPlayerHero:Purge( true, true, false, true, true )

	-- Remove all items (including TP scroll slot and neutral item slot)
	...
	-- Re-give starting items (fresh copies — no charges used)
	...
	-- Reset health / mana to scenario starting values
	if self.hScenario.StartingHealthPct ~= nil then
		hPlayerHero:SetHealth( hPlayerHero:GetMaxHealth() * self.hScenario.StartingHealthPct )
	elseif self.hScenario.StartingHealth ~= nil then
		hPlayerHero:SetHealth( self.hScenario.StartingHealth )
	else
		hPlayerHero:SetHealth( hPlayerHero:GetMaxHealth() )
	end
	if self.hScenario.StartingMana ~= nil then
		hPlayerHero:SetMana( self.hScenario.StartingMana )
	else
	...
	-- Move back to starting position if defined, otherwise
	-- find the goodguys spawn point
	if self.hScenario.StartingPosition ~= nil then
