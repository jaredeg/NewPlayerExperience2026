...
function CDotaNPXScenario_Courier:OnHeroFinishSpawn( hHero, hPlayer )
	...
	-- Strip cosmetic wearables from the player hero to prevent ERROR models.
	-- Player's equipped cosmetics aren't precached in custom games.
	self:StripCosmetics( hHero )

	self.hCourierSpawn = Entities:FindByName( nil, "courier_spawn_radiant" )
	self.hCourier = CreateUnitByName( "npc_dota_courier", self.hCourierSpawn:GetAbsOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS )
	self:ShowWizardTip( "scenario_courier_wizard_tip_buying", 15.0 )
	if self.hCourier then
		self.hCourier:UpgradeCourier( 10 )
		self.hCourier:SetControllableByPlayer( hHero:GetPlayerID(), true )
		self.hCourier:SetOwner( hHero )
	end
...
function CDotaNPXScenario_Courier:StripCosmetics( hUnit )
	-- Force base model and strip ALL cosmetic wearables to prevent ERROR models.
	if hUnit == nil or hUnit:IsNull() then return end

	-- Force the base model so the engine stops trying to load cosmetic replacements
	local szUnitName = hUnit:GetUnitName()
	if szUnitName == "npc_dota_hero_night_stalker" then
		hUnit:SetOriginalModel( "models/heroes/night_stalker/night_stalker.vmdl" )
		hUnit:SetModel( "models/heroes/night_stalker/night_stalker.vmdl" )
	end

	local hChildren = hUnit:GetChildren()
	if hChildren then
		for _, hChild in pairs( hChildren ) do
			if hChild and not hChild:IsNull() and hChild:GetClassname() == "dota_item_wearable" then
