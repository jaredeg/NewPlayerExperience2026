...
function CDOTANPXScenario_Economy:SetupScenario()
	...
	GameRules:GetGameModeEntity():SetFixedRespawnTime( 3.0 )
...
function CDOTANPXScenario_Economy:SetupTask_CompleteEchoSabre()
	...
	retrieveVoidStoneTask.StartTask = function( task )
		...
		if hPlayer and hSpawnEnt then
			local hCourier = CreateUnitByName( "npc_dota_courier", hSpawnEnt:GetAbsOrigin(), true, nil, nil, DOTA_TEAM_GOODGUYS )
			if hCourier then
				hCourier:UpgradeCourier( 12 )
				hCourier:SetControllableByPlayer( 0, true )
				hCourier:SetOwner( PlayerResource:GetSelectedHeroEntity( 0 ) )
			end
	...
	deliverVoidStoneTask.CompleteTask = function( task )
		...
		-- Send courier back to fountain and hide it
		local hCourier = Entities:FindByClassname( nil, "npc_dota_courier" )
		if hCourier and not hCourier:IsNull() then
			local hFountain = FindSpawnEntityForTeam( DOTA_TEAM_GOODGUYS )
			if hFountain then
				hCourier:SetAbsOrigin( hFountain:GetAbsOrigin() )
			end
			hCourier:Stop()
			hCourier:AddNoDraw()
		end
...
function CDOTANPXScenario_Economy:OnSpawnerFinished( event )
	if event.spawner_name == "sf_spawner" then
		-- self.SfSpawner is not assigned yet (constructor hasn't returned).
		-- Find the SF hero directly and freeze it offmap.
		local hSF = PlayerResource:GetSelectedHeroEntity( 1 )
		if hSF and not hSF:IsNull() then
			-- Use the spawner entity's position as the origin (hero may already
			-- be at 10000,10000 from a previous run).
			local hSpawnerEnt = Entities:FindByName( nil, "sf_spawner" )
			hSF.bFrozen = true
			hSF:AddNewModifier( nil, nil, "modifier_invulnerable", {} )
			if hSpawnerEnt then
				hSF.vSpawnOrigin = hSpawnerEnt:GetAbsOrigin()
			else
				hSF.vSpawnOrigin = hSF:GetAbsOrigin()
			end
			hSF:SetAbsOrigin( Vector( 10000, 10000, 0 ) )
		end
	end
end
...
function CDOTANPXScenario_Economy:IntroduceScenario()
	self:RevealSpawnedUnit( self.SfSpawner )
	AddFOWViewer( DOTA_TEAM_GOODGUYS, self.SfSpawner:GetSpawnedUnits()[1]:GetAbsOrigin(), 500, 6.0, false )
	...
	if self.SfSpawner:GetSpawnedUnits()[1] ~= nil then
		...
		self:IntroduceUnit( self.SfSpawner:GetSpawnedUnits()[1], 6.0, 30, 90, 600, 0.1, 120 )
	end	

	self:ScheduleFunctionAtGameTime( GameRules:GetGameTime() + 5.5, function()
		self:IntroduceUnit( self.hPlayerHero, 2.0, 30, 90, 600, 0.1, 120 )
	end )

	self:ScheduleFunctionAtGameTime( GameRules:GetGameTime() + 7.0, function()
		self:EndIntroduceScenario()
		self.hPlayerHero:SetForceAttackTarget( nil )
		self.flTimeToIntroduceTask = GameRules:GetGameTime() + 1.0
...
function CDOTANPXScenario_Economy:OnThink()
	if self.bIntroductionComplete == false and self.hPlayerHero ~= nil and self.SfSpawner ~= nil and self.SfSpawner:GetSpawnedUnits()[1] ~= nil then
