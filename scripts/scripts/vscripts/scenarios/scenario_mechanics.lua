...
function CDotaNPXScenario_Mechanics:OnHeroFinishSpawn( hHero, hPlayer )
	...
	-- Log all abilities for debugging and learn Moon Glaives
	local bFoundMoonGlaive = false
	for i = 0, self.hHero:GetAbilityCount() - 1 do
		local hAbility = self.hHero:GetAbilityByIndex( i )
		if hAbility then
			local szName = hAbility:GetAbilityName()
			printf( "  Ability[%d]: %s (level %d, hidden=%s)", i, szName, hAbility:GetLevel(), tostring( hAbility:IsHidden() ) )

			-- Match any ability containing "moon_glaive" 
			if string.find( szName, "moon_glaive" ) and hAbility:GetLevel() == 0 then
				printf( "  >> Force-learning Moon Glaives: %s at index %d", szName, i )
				-- Give a temporary ability point, use UpgradeAbility (more reliable
				-- than SetLevel for some abilities), then zero out points after
				self.hHero:SetAbilityPoints( 1 )
				self.hHero:UpgradeAbility( hAbility )
				bFoundMoonGlaive = true
			end
		end
	end

	if not bFoundMoonGlaive then
		printf( "WARNING: Could not find any moon_glaive ability to learn!" )
	end

	-- Zero out ability points AFTER learning moon glaive
	-- (player must earn their own point to learn Lucent Beam)
...
function CDotaNPXScenario_Mechanics:OnTaskStarted( event )
	...
	elseif event.task_name == "learn_lucent_beam" then
		-- Find the actual UI slot index for lucent beam dynamically
		-- (innate abilities may shift the panel layout)
		local nAbilitySlot = self:FindAbilityPanelIndex( "luna_lucent_beam" )
		local szSlot = "Ability" .. nAbilitySlot
		printf( "learn_lucent_beam: using panel slot %s", szSlot )
		self:ShowUIHint( szSlot .. " LevelUpButton", "scenario_mechanics_ui_tip_learn_lucent_beam", 0.0, nil )
		self:ShowWizardTip( "scenario_mechanics_wizard_tip_intro_abilities", 25.0, { "luna_lucent_beam" } )
	elseif event.task_name == "use_lucent_beam" then
		local nAbilitySlot = self:FindAbilityPanelIndex( "luna_lucent_beam" )
		local szSlot = "Ability" .. nAbilitySlot
		printf( "use_lucent_beam: using panel slot %s", szSlot )
		self:ShowUIHint( szSlot .. " AbilityButton", "scenario_mechanics_ui_tip_use_lucent_beam", 0.0, nil )
		self:ShowWizardTip( "scenario_mechanics_wizard_tip_intro_casting", 25.0, { "luna_lucent_beam" } )
...
--------------------------------------------------------------------------------
-- Find the UI panel index for an ability by name.
-- Iterates the hero's ability list and counts only "normal" abilities
-- (skipping hidden/innate ones) to determine the HUD slot index.
function CDotaNPXScenario_Mechanics:FindAbilityPanelIndex( szAbilityName )
	local hHero = self:GetPlayerHero()
	if hHero == nil then return 0 end

	-- First, try to find the ability and get its raw index
	for i = 0, hHero:GetAbilityCount() - 1 do
		local hAbility = hHero:GetAbilityByIndex( i )
		if hAbility and hAbility:GetAbilityName() == szAbilityName then
			printf( "FindAbilityPanelIndex: %s found at raw index %d", szAbilityName, i )
			-- In Dota 2, the panel index typically matches the ability index
			-- for non-innate abilities. Innate abilities are in separate slots (16+).
			return i
		end
	end

	printf( "WARNING: FindAbilityPanelIndex could not find %s, defaulting to 0", szAbilityName )
	return 0
end
