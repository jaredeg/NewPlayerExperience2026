...
function CDotaNPXScenario_Navigation:SetupScenario()
	...
	-- Disable newer rune types added since initial release
	if DOTA_RUNE_SHIELD ~= nil then
		GameRules:GetGameModeEntity():SetRuneEnabled( DOTA_RUNE_SHIELD, false )
	end
	if DOTA_RUNE_WATER ~= nil then
		GameRules:GetGameModeEntity():SetRuneEnabled( DOTA_RUNE_WATER, false )
	end
	if DOTA_RUNE_WISDOM ~= nil then
		GameRules:GetGameModeEntity():SetRuneEnabled( DOTA_RUNE_WISDOM, false )
	end
...
function CDotaNPXScenario_Navigation:OnTaskStarted( event )
	...
	elseif event.task_name == "move_to_location_6" then
		-- Haste Rune — manually spawn since all natural rune spawning is disabled
		local vRunePos = self.hHintLoc4:GetAbsOrigin()
		-- Remove any existing rune first (in case of restart)
		if self.hHashteRune then
			UTIL_RemoveImmediate( self.hHashteRune )
		end
		self.hHashteRune = CreateRune( vRunePos, DOTA_RUNE_HASTE )
