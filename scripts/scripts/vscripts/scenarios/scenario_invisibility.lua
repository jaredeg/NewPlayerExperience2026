...
function CDotaNPXScenario_Invisibility:OnSpawnerFinished( event )
	CDotaNPXScenario.OnSpawnerFinished( self, event )

	local hSpawner = self:GetSpawner( event.spawner_name )
	if hSpawner == nil then return end

	if event.spawner_name == "bounty_hunter_spawner" then
		self.hBountyHunterSpawner = hSpawner
		self:FreezeSpawnedUnit( hSpawner )
	elseif event.spawner_name == "riki_spawner" then
		self.hRikiSpawner = hSpawner
		self:FreezeSpawnedUnit( hSpawner )
	end
end
...
function CDotaNPXScenario_Invisibility:OnTaskCompleted( event )
	...
	if Task:GetTaskName() == "buy_dust" then
		...
		if self.hBountyHunterSpawner then self:RevealSpawnedUnit( self.hBountyHunterSpawner ) end
	...
	if Task:GetTaskName() == "destroy_enemy_ward" then
		...
		if self.hRikiSpawner then self:RevealSpawnedUnit( self.hRikiSpawner ) end
