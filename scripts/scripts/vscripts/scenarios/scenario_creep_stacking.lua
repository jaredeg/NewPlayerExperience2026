...
function CDotaNPXScenario_CreepStacking:OnHeroFinishSpawn( hHero, hPlayer )
	...
	-- Force base model and strip cosmetic wearables to prevent ERROR models
	hHero:SetOriginalModel( "models/heroes/vengeful/vengeful.vmdl" )
	hHero:SetModel( "models/heroes/vengeful/vengeful.vmdl" )
	local hChildren = hHero:GetChildren()
	if hChildren then
...
function CDotaNPXScenario_CreepStacking:OnTaskCompleted( event )
	-- Guard: if already failed, ignore all further task completions
	if self.bScenarioFailed then
		return
	end

	if event.success == 0 then
		self.bScenarioFailed = true
		FireGameEvent( "timer_hide", {} )
		self:OnScenarioComplete( false, event.failure_reason )
		return
	end
