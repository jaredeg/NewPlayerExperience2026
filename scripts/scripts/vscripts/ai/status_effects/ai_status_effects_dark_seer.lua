...
function CStatusEffectsDarkSeerBot:BotThink()
	...
	if self.me:FindModifierByName("modifier_rod_of_atos_debuff") ~= nil then
		...
		if self.hAttackTarget and not self.hAttackTarget:IsNull() and self.bSvenActivated == false then
	...
	if self.hAttackTarget == nil or self.hAttackTarget:IsNull() then
		self:FindBestTarget()
	end

	if self.hAttackTarget and not self.hAttackTarget:IsNull() and self.hAttackTarget:IsAlive() then
  		ExecuteOrderFromTable( {
			UnitIndex = self.me:entindex(),
			OrderType = DOTA_UNIT_ORDER_ATTACK_TARGET,
			TargetIndex = self.hAttackTarget:entindex(),
