...
function CDotaSpawner:constructor( szSpawnerNameInput, rgUnitsInfoInput, hScenario, bSpawnOnPrecacheComplete )
	...
	if self.hScenario then
		self.hScenario:AddSpawner( self )
	end

	-- If async precache has already completed (resources cached from the
	-- initial Precache() phase), spawn immediately.
	if self.bSpawnOnPrecacheComplete and self:IsFullyPrecached() then
		self:SpawnUnits()
	end
end
...
function CDotaSpawner:PrecacheUnits()
	...
	-- Do NOT call PrecacheUnitByNameAsync here.  It is only safe during the
	-- initial Precache() loading phase.  Calling it during SetupScenario
	-- (which runs in CUSTOM_GAME_SETUP state) crashes the engine.
	-- Heroes are precached via g_UnitPrecache / PrecacheUnitByNameSync in
	-- addon_game_mode.lua Precache(), which runs on every map load
	-- including restarts.
end
