...
function CStatusEffectsSvenBot:BotThink()
	...
	if self.nBotState == SVEN_BOT_STATE_INACTIVE then
		-- Scenario handles cleanup via RemoveSpawnedUnits; just stop acting.
	...
	if self.nBotState == SVEN_BOT_STATE_ATTACK then
		...
		if self.hAttackTarget == nil or self.hAttackTarget:IsNull() or not self.hAttackTarget:IsAlive() then
