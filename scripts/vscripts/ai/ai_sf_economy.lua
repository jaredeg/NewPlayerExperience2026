...
function SFEconomyThink()
	...
	if thisEntity.bHasPickedUpRune == false then
		local hRune = Entities:FindByClassname( nil, "dota_item_rune_bounty" )
		if hRune then
	...
	-- Guard GetKills: GetPlayerOwnerID() returns -1 for AI units spawned without a
	-- player slot (e.g. scenario-spawned SF in the level-10 phase). Calling
	-- PlayerResource:GetKills(-1) returns nil, and nil >= 1 crashes with a Lua
	-- comparison error. Use 0 as a safe fallback for ownerless AI units.
	local nSFPlayerID = thisEntity:GetPlayerOwnerID()
	local nSFKills = ( nSFPlayerID >= 0 ) and PlayerResource:GetKills( nSFPlayerID ) or 0

	if nSFKills >= 1 and thisEntity:GetAggroTarget() == nil and thisEntity:GetAbsOrigin() ~= thisEntity.vHomePosition then
...
function RebuildRazeTargets()
	...
	local nMaxRange = nRaze3Range + nRazeRadius
	local NearbyUnits = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetAbsOrigin(), thisEntity, nMaxRange, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false )

	for _,Unit in pairs( NearbyUnits ) do
		local bIsHero = Unit:IsHero()
		local bIsCreep = Unit:IsCreep()
		local flDistToMe = (thisEntity:GetAbsOrigin() - Unit:GetAbsOrigin()):Length2D()

		if bIsHero or ( bIsCreep and Unit:GetHealth() < flRazeDamage ) then
			if flDistToMe > ( nRaze1Range - nRazeRadius ) and flDistToMe < ( nRaze1Range + nRazeRadius ) then
				table.insert( thisEntity.Raze1Targets, Unit )
			end

			if flDistToMe > ( nRaze2Range - nRazeRadius ) and flDistToMe < ( nRaze2Range + nRazeRadius ) then
				table.insert( thisEntity.Raze2Targets, Unit )
			end

			if flDistToMe > ( nRaze3Range - nRazeRadius ) and flDistToMe < ( nRaze3Range + nRazeRadius ) then
				table.insert( thisEntity.Raze3Targets, Unit )
			end
		end
	end
end
...
function Shadowraze( hRaze )
	...
	--thisEntity.LaningAI:InvalidateCurrentAction() -- LaningAI unused in economy variant
	...
	--thisEntity.LaningAI:SetCastingAbility( hRaze:GetAbilityName(), nil, 3.0 ) -- LaningAI unused in economy variant
	return 0.1
end
