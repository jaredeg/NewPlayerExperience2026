...
function CDotaNPXScenario_EulsSetup:SetupScenario()
	...
	-- Pre-spawn SF during setup so cosmetics load with valid precache context.
	-- OnSpawnerFinished freezes him offmap; OnTaskCompleted reveals him.
	CDotaSpawner( "enemy_waypoint_1",
	{
		{
...
function CDotaNPXScenario_EulsSetup:OnTaskCompleted( event )
	...
	if Task:GetTaskName() == "move_to_location" then
		-- Reveal SF (pre-spawned frozen in SetupScenario)
		if self.hSFSpawner then
			self:RevealSpawnedUnit( self.hSFSpawner )
		end
...
function CDotaNPXScenario_EulsSetup:OnSpawnerFinished( event )
	...
	if event.spawner_name == "enemy_waypoint_1" then
		self.hSFSpawner = hSpawner
