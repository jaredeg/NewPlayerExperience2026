...
function CDotaNPXScenario_Regen:SetupScenario()
	...
	-- Pre-spawn Lina (ally) so her model is precached
	...
	-- Pre-spawn Lion (enemy) so his model is precached
...
function CDotaNPXScenario_Regen:OnSpawnerFinished( event )
	...
	for _, spawner in pairs( self.rgSpawners ) do
		...
		if hUnit and not hUnit.bFrozen then
			...
			if szName == "npc_dota_hero_lina" then
				printf( "Freezing pre-spawned Lina" )
				self.hLinaSpawner = spawner
				self:FreezeSpawnedUnit( spawner )
				break
			elseif szName == "npc_dota_hero_lion" then
				printf( "Freezing pre-spawned Lion" )
				self.hLionSpawner = spawner
				self:FreezeSpawnedUnit( spawner )
				break
			end
		end
...
function CDotaNPXScenario_Regen:OnTaskCompleted( event )
	...
	elseif event.task_name == "buy_regen" then
		-- Reveal pre-spawned Lina
		if self.hLinaSpawner then
			self:RevealSpawnedUnit( self.hLinaSpawner )
			local hLina = self.hLinaSpawner:GetSpawnedUnits()[ 1 ]
			if hLina then
				hLina:SetHealth( 200 )
				self:GetTask("regen_lina"):SetRegenUnit( hLina )
			end
		end
    elseif event.task_name == "share_tango" then
		self:HideUIHint()
		-- Reveal pre-spawned Lion
		if self.hLionSpawner then
			self:RevealSpawnedUnit( self.hLionSpawner )
		end
