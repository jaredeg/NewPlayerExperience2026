...
function CDotaNPXTask_StackTarget:OnThink()
	...
	if self.bIsComplete == false then
		...
		if self.hTarget and self.hTarget:IsAlive() then
			...
			-- Target died — treat as if camp is empty (success) if no time requirement,
			-- otherwise just let the timer run out naturally
			if not self.hTaskInfo.TaskParams.TimeRequirement then
				self:CompleteTask( true )
				self.bIsComplete = true
			end
		end
