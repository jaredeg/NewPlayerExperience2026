...
function Precache( context )
	...
	-- All hero and unit precaching is handled via g_UnitPrecache in
	-- precache.lua.  Do NOT add explicit PrecacheUnitByNameSync calls here
	-- for heroes — double-precaching corrupts SF/Nevermore in economy.

	-- Precache courier model paths (engine needs these explicit resource refs).
	PrecacheResource( "model", "models/courier/default_courier/default_courier.vmdl", context )
	PrecacheResource( "model", "models/courier/default_courier/default_courier_flying.vmdl", context )
	PrecacheResource( "model", "models/props_gameplay/donkey.vmdl", context )
	PrecacheResource( "model", "models/props_gameplay/donkey_flying.vmdl", context )

	-- Precache equipped cosmetic models.  Bot heroes inherit the host player's
	-- cosmetics via AddBotPlayerWithEntityScript.  These models exist in base
	-- Dota 2 but are NOT in the addon's resource manifest, causing
	-- "nonresident asset" crashes when they're referenced after a restart.
	-- QoP arcana + cosmetics:
	PrecacheResource( "model", "models/items/queenofpain/queenofpain_arcana/queenofpain_arcana.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/queenofpain_arcana/queenofpain_arcana_wings.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_ti8_weapon/qop_ti8_weapon.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_ti8_weapon/qop_ti8_weapon_arcrefit.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_deadly_whisper_head/qop_deadly_whisper_head.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_deadly_whisper_head/qop_deadly_whisper_head_refit.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_2022_immortal/qop_2022_immortal.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_2022_immortal/qop_2022_immortal_legs.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_2022_immortal/qop_2022_immortal_arcana_refit.vmdl", context )
	PrecacheResource( "model", "models/items/queenofpain/qop_2022_immortal/qop_2022_immortal_legs_arcana_refit.vmdl", context )
	-- WK arcana + cosmetics:
	PrecacheResource( "model", "models/items/wraith_king/arcana/wraith_king_arcana.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_weapon/wk_ti8_weapon.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_back/wk_ti8_back.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_shoulder/wk_ti8_shoulder.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_head/wk_ti8_head.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_weapon/wk_ti8_weapon_arcana_refit.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_back/wk_ti8_back_arcana_refit.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_shoulder/wk_ti8_shoulder_arcana_refit.vmdl", context )
	PrecacheResource( "model", "models/items/wraith_king/wk_ti8_head/wk_ti8_head_arcana_refit.vmdl", context )

	-- Custom units are also in g_UnitPrecache — no explicit calls needed.
end
...
function CDotaNPX:InitGameMode()
	-- Prevent the engine from combining cosmetic wearable meshes into
	-- the hero model.  Combined models reference cosmetic assets that
	-- are not in the addon's resource manifest, causing crashes.
	SendToServerConsole( "dota_combine_models 0" )

	self.CurrentScenario = nil
	self.flNextTimerConsoleNotify = -1
	self.AbilityKV = LoadKeyValues( "scripts/npc_abilities.txt" )
...
function CDotaNPX:RestartScenario()
	...
	-- NOTE: Do NOT call ResetToCustomGameSetup() here.
	-- It purges loaded cosmetic model resources, causing bot hero entities
	-- to break on re-creation.  Restart() handles all necessary cleanup
	-- and calls SetupScenario() directly.
